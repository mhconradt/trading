...
import json
import time  # noqa: F401 (used in _keepalive)
    ...
    def _connect(self):
        ...
        if self.channels is None:
            self.channels = [{"name": "ticker",
                              "product_ids": [product_id for product_id in
                                              self.products]}]
        ...
        if self.auth:
            ...
            auth_headers = get_auth_headers(timestamp, message, self.api_key,
                                            self.api_secret,
                                            self.api_passphrase)
...
if __name__ == "__main__":
    ...
    class MyWebsocketClient(WebsocketClient):
        ...
        def on_open(self):
            ...
            self.products = [p['id'] for p in products if
